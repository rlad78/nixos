...
in
{
  ...
  config = {
    networking.hosts = (
      lib.attrsets.optionalAttrs cfg.tailscale (
        tail-hosts (has-tail-ip (removeAttrs hosts [ config.networking.hostName ]))
      )
